$(function(){
	...
	//获取content对象里面，数据的数量
	var number_of_items = $(".modify-hotel-row").length;

	//计算页面显示的总页数
	var pageCount = Math.ceil(number_of_items/show_per_page);

	//隐藏该对象下面的所有子元素
    $(".modify-hotel-row").css('display', 'none');

	//显示第n（show_per_page）元素
    $(".modify-hotel-row").slice(0, show_per_page).css('display', 'block');

	//隐藏域默认值
	$('#qa_start_page').val(0);
	$('#qa_current_page').val(0);
	$('#qa_show_per_page').val(show_per_page);
	$('#qa_end_page').val(pageCount);
	...
	$("body").on("click","#pageQA li",function(){
		...
		$(".modify-hotel-row").css('display', 'none').slice(start_from_this, end_on_this).css('display', 'block');
